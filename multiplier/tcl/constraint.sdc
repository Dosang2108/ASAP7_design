...
# Ð? b?t d?nh xung nh?p (Uncertainty) bù tr? cho Jitter và Skew
set_clock_uncertainty 50 [all_clocks]

# Ð? tr? t? ngu?n c?p Clock d?n chân Flip-Flop (Latency)
set_clock_latency -min 100 [all_clocks]
set_clock_latency -max 150 [all_clocks]

###############################################################
# Input constraints 
###############################################################
set_input_transition -min 10 [remove_from_collection [all_inputs] [get_ports $CLK]]
set_input_transition -max 40 [remove_from_collection [all_inputs] [get_ports $CLK]]
...
set_load 5 -pin_load [all_inputs]
...
set_load 10 -pin_load [all_outputs]
